...
def a_star_search(problem: Problem):
    """
    A* search algorithm formula was used from Page 89 of the course textbook
    f(n) = g(n) + h(n) where:
    ...
    # Create the initial node and set its path_cost (g(n)) = 0
    ...
    # The frontier uses the custom priority queue TS, ordered by f(n) = g(n) + h(n)
    # Ties are broken by node.state (smaller ID first), then by chronological order (NOTE 2 requirement)
    ...
    # from the frontier and had their children generated), prevents
