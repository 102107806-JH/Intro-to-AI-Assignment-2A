...
def depth_first_search(problem, depth_limit, cycle_depth_limit):
    ...
    while not frontier.is_empty():  # Keep going until the frontier is empty #
        node = frontier.pop()  # Pop the top of the stack off #
        if problem.is_goal(node.state):  # If we are at the goal node return the node #
            return node

        if node.node_depth < depth_limit and is_cycle(node, cycle_depth_limit) == False:
            # Only enable expansion if the node is less than depth limit
            # Nodes at the depth limit will still be explored. Also check for cycles #

            for child in sorted(expand(problem, node), key=lambda node: node.state, reverse=True):
                # Children need to be explored in ascending order because stack is used need to push higher nodes first
                # thus list needs reversing by state #
...
def expand(problem, node):
    ...
    for action in problem.actions(state):  # Go through all the states #
        ...
        children.append(Node(state=new_state, parent=node, action=action, path_cost=path_cost,
                             node_depth=node.node_depth + 1))  # Append a new child node using the node constructor #
