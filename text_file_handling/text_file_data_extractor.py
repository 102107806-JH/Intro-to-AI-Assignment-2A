...
class TextFileDataExtractor:
    ...
    def extract_text_file_data(self):
        ...
        adjacency_list_graph = self._insert_vertexes_into_adjacency_list(adjacency_list_graph)  # Insert vertexes into adjacency list graph from text file #

        adjacency_list_graph = self._insert_edges_into_adjacency_list(adjacency_list_graph)  # Insert vertex edges from text file #
        ...
        return ExtractedTextFileData(adjacency_list_graph, origin, destinations_list)  # Return the data in a wrapper object #
    ...
    def _insert_edges_into_adjacency_list(self, adjacency_list_graph):
        ...
        for c in edges_data_string:
            ...
            if c == ",":  # Origin id has been found
                ...
                adjacency_list_graph.insert_edge_into_vertex(origin_id, destination_id, cost)  # Insert a new edges into the graph #
                data_buffer = ""
                continue
    ...
    def _extract_origin(self):
        ...
        origin_data_string = self._file_contents[origin_text_end: destinations_text_start]  # Slice out the origin data #
    ...
    def _extract_destinations(self):
        ...
        destinations_text_end = self._file_contents.find("Destinations:") + len("Destinations:")  # Start index of the destinations data #
        destinations_data_string = self._file_contents[destinations_text_end: -1]  # Slice out the destinations data (no end index needs to be calculated because it goes up string end) #
        destinations_data_string = destinations_data_string.replace(" ", "").replace("\n", "")  # Removing out all spaces and newline chars #


        destinations_data_string += ";"  # Formating string to make it easier to deal with #

        data_buffer = ""  # Data buffer #
        destinations_list = []  # A list that will store all the destinations as intergers #
        for c in destinations_data_string:
            ...
            if c == ";":  # Have found a destination #
                ...
                continue  # Dont want the current char added to the data buffer #
