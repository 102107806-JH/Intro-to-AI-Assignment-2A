...
def weighted_a_star(problem, weight=1):
    ...
    while not frontier.is_empty():  # Continue until frontier is empty #
        ...
        for child in expand(problem, node, weight):  # Expand popped nodes children #
            ...
            if reached.get(state, False) == False or child.path_cost < reached[state].path_cost:
...
def expand(problem, node, weight):
    state = node.state  # Set the state of the parent node #
    ...
    for action in problem.actions(state):  # Go through all the states #
        ...
        heuristic_cost = weight * problem.state_distance_to_goal(
            new_state)  # Get the heuristic (straight line distance) for the expanded node and multiply it by the weight
        children.append(Node(state=new_state, parent=node, action=action, path_cost=path_cost,
                             heuristic_cost=heuristic_cost))  # Append a new child node using the node constructor #
